...
class LUAFM(nn.Module):
    ...
    def prepare(self, x, y):
        x = self.prepare_x(x, y)    # low
        y = self.prepare_y(x, y)    # hight
        return x, y
    ...
    def fuse(self, x, y):
        ...
        # vis.visualize_single_sample(
        #     x, y, atten,
        #     save_path='feature_fusion_visualization.png'
        # )
...
class BSSNet_T(BaseModule):
    def __init__(self,
        ...
        #self.graphdiff = CrossGraphDifference(in_channels=64)

        # I Branch 语义分支
        self.i_branch_layers = nn.ModuleList()
        for i in range(3):    #E1 (64->128)  E2(128->256)  E3(256->512)
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor]]:
        x = self.stem(x)
        x_b = self.b_branch_layers[0](x)    #B1
        x_d = self.d_branch_layers[0](x)
        x_i_3 = self.relu(self.i_branch_layers[0](x))   #E1 out
        comp_b = self.compression_b(x_i_3)
        x_b = self.uafm_ib(x_b, comp_b)
        comp_d = self.compression_d(x_i_3)
        x_d = self.uafm_id(x_d, comp_d)

        # diff_map, edge_diffs, total_diff = self.graphdiff(x_b, x_d)
        # simple_visualization(x_b, x_d, diff_map, sample_idx=0)

        if self.training:
            tmp_xb = x_b.clone()
            tmp_xd = x_d.clone()

        x_b = self.b_branch_layers[1](x_b)  # B2
        ...
        out = self.uafm3(x_i_3, out)    # 主干语义分支输出
