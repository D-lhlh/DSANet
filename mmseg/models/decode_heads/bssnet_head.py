...
class BSSNet_Head(BaseDecodeHead):
    ...
    def forward(
        ...
        if self.training:
            ...
            assert len(x) == 3  #训练过程的三个损失：边界损失，空间损失和分割损失
            x_b, x_d, fuse_b_d = x
            fuse_b_d = self.fuse_b_d_head(fuse_b_d, self.cls_seg) #使用头处理输出
    ...
    def loss_by_feat(self, seg_logits: Tuple[Tensor],
        ...
        bd_label = bd_label.squeeze(1)      #边界标签

        loss['loss_sem_i'] = self.loss_decode[0](sem_i_logit, sem_label)    #分割损失
        loss['loss_bd'] = self.loss_decode[1](b_logit, bd_label)    #边界损失

        filler = torch.ones_like(sem_label) * self.ignore_index
        sem_bd_label = torch.where(torch.sigmoid(b_logit[:, 0, :, :]) > 0.8, sem_label, filler) #sigmoid将边界预测转换为概率值，认为概率值大于0.8的为边界
        #    torch.where(condition, x, y)    条件成立：从x取值，不成立：从y取值
        loss['loss_sem_bd'] = self.loss_decode[2](sem_i_logit, sem_bd_label)
        # ohem输入 预测的类别概率(bs,nc,h,w)，真实标签squeeze(1)(bs,h,w)

        loss['loss_detail'] = self.loss_detail(d_logit, inputs) * self.loss_detail_weight   #空间损失 L1loss对应值相减的绝对值 取均值

        loss['acc_seg'] = accuracy(sem_i_logit, sem_label, ignore_index=self.ignore_index)
        return loss
